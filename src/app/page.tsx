    ...
    <div className="animate-fade-in">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-slate-400 dark:text-slate-500 mt-1">AI-powered company research & GTM intelligence</p>
        </div>
      ...
      {!loading && !error && projects.length > 0 && (
        <div className="mb-4 flex items-center justify-between">
          <h2 className="text-sm font-semibold text-slate-600 dark:text-slate-300">Recent Projects</h2>
          <Link href="/projects" className="text-xs text-slate-400 dark:text-slate-500 hover:text-[#3289FF]">
            View all projects →
          </Link>
        </div>
      )}
          ...
          <div className="grid grid-cols-3 gap-4">
            {[...Array(3)].map((_, i) => (
              <div key={i} className="card p-5">
                <div className="h-7 w-12 bg-slate-100 dark:bg-slate-800 rounded animate-pulse mb-2" />
                <div className="h-3 w-20 bg-slate-50 dark:bg-slate-800/60 rounded animate-pulse" />
              </div>
            ))}
          </div>
          <div className="h-5 w-28 bg-slate-100 dark:bg-slate-800 rounded animate-pulse" />
          {[...Array(3)].map((_, i) => (
          ...
          {activeProjects.length > 0 ? (
            <>
              ...
              {activeProjects.length > 4 && (
                <Link href="/projects" className="block text-center text-xs text-slate-400 dark:text-slate-500 hover:text-[#3289FF] py-2">
            ...
            <div className="card p-8 text-center">
              <p className="text-sm text-slate-400 dark:text-slate-500">No active projects. {archivedProjects.length > 0 ? <Link href="/projects" className="text-[#3289FF] hover:underline">See archived</Link> : ""} or create a new one.</p>
            </div>
          )}
        </div>
      )}
